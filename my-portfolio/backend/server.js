...
app.post(
    ...
    async (req, res) => {
        try {
            ...
            await uploadImageGalleryToPortfolioId(
                portfolioId,
                galleryImageUrls
            );

            res.status(200).json({
                message: "Portfolio and gallery images uploaded successfully",
                portfolioId,
            });
        } catch (err) {
            console.error(err);
            res.status(500).json({
                message:
                    "Error creating portfolio and uploading gallery images",
            });
        }
    }
);
...
app.post(
    "/uploadMultipleGalleryImagesToPortfolioId",
    upload.array("gallery_images", 9),
    async (req, res) => {
        try {
            ...
            if (!portfolio_id || !files || files.length === 0) {
                return res.status(400).json({
                    message:
                        "portfolio_id and at least one image file are required.",
                });
            }
...
app.delete("/deleteGalleryById/:id", async (req, res) => {
    try {
        ...
        const galleryPublicId = extractCloudinaryPublicId(
            gallery_img_data[0].img
        );
        if (galleryPublicId) {
        ...
        res.status(500).json({
            message: `Error deleting gallery ID ${id}`,
            error: error.message,
        });
    }
});
...
app.put("/updateSkillTypeById/:id", async (req, res) => {
    ...
    if (!id || !name || !color) {
        return res
            .status(400)
            .send({ message: "Skill type ID, name, and color are required" });
    }
    try {
...
app.post("/addSkillTypeToPortfolio", async (req, res) => {
    ...
    if (!portfolio_id || !skill_type_ids || !Array.isArray(skill_type_ids)) {
        return res.status(400).json({
            message: "portfolio_id and skill_type_ids (array) are required",
        });
    }
    ...
    try {
        ...
        res.status(200).send(
            `Skill types [${skill_type_ids.join(
                ", "
            )}] added to portfolio with ID ${portfolio_id} successfully`
        );
    } catch (error) {
...
// GET image count in a specific Cloudinary folder
app.get("/getImageCountByFolder/:folderName", async (req, res) => {
    ...
    try {
        do {
            const response = await axios.get(
                `https://api.cloudinary.com/v1_1/${cloudName}/resources/image`,
                {
                    ...
                    params: {
                        type: "upload",
                        prefix: `${folderName}/`, // Ensure this has a trailing slash!
                        max_results: 500,
        ...
        res.json({ folder: folderName, imageCount: count });
    } catch (error) {
        console.error(
            "Cloudinary error:",
...
app.post("/register", async (req, res) => {
    try {
        ...
        const registrationResult = await adminRegister(
            username,
            encryptedPassword
        );
